...
mu_set = 0:0.5:2; % mean of customers' utilities from good one and two
...
Ave_gain_known = nan(length(mu_set),1);
Ave_gain_unknown = nan(length(mu_set),1);
p_star = nan(length(mu_set),1);
p_star_unknown_Ave = nan(length(mu_set),1);

for z = 1:length(mu_set)
    mu = mu_set(z);
    mu_vec = [mu;mu];
...
save('Mean_Main.mat','Ave_gain_unknown','Ave_gain_known','p_star_unknown_Ave','p_star')
toc % stop timer
delete(gcp('nocreate'))