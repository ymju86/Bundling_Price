...
sig_set = 1:3; % variance of utilities for good one and two

sim = 1000; % number of simulations

Ave_gain_known = nan(length(sig_set),1);
Ave_gain_unknown = nan(length(sig_set),1);
p_star = nan(length(sig_set),1);
p_star_unknown_Ave = nan(length(sig_set),1);

for z = 1:length(sig_set)
    sig = sig_set(z);
    mu_vec = [mu;mu];
...
save('Variance.mat','Ave_gain_unknown','Ave_gain_known','p_star_unknown_Ave','p_star')
toc % stop timer
delete(gcp('nocreate'))