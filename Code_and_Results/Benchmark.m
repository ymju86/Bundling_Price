...
sig12 = 0; % covariance between utility from good one and two
...
for i = 1:sim
    ...
    index1_b = utility(:,1) > p1 & utility(:,2) < p_star - p1 ;
    index2_b = utility(:,1) < p_star - p2 & utility(:,2) > p2 ;
    index3_b = utility(:,1) + utility(:,2) > p_star & utility(:,1) > p_star - p2 & utility(:,2) > p_star - p1 ;
    Sale_b = (p1*sum(index1_b) + p2*sum(index2_b) + p_star*sum(index3_b));
...
Ave_gain_known = mean(gain_known);
...
Ave_gain_unknown = mean(gain_unknown);
p_star_unknown_Ave = mean(p_star_unknown);

save('Benchmark.mat','Ave_gain_unknown','Ave_gain_known','p_star_unknown_Ave','p_star')
toc % stop timer
delete(gcp('nocreate'))